...
export const AuthProvider = ({ children }: { children: React.ReactNode }) => {
    ...
    const checkAuth = async () => {
        try {
            // Gọi API lấy profile mới nhất của tài xế
            const res = await axiosClient.get('/driver/profile');
            const driverProfile = res.data; 

            // Cập nhật lại state user với status mới (VD: từ INCOMPLETE sang PENDING_APPROVAL)
            setUser((prevUser) => {
                if (!prevUser) return null;
                
                const updatedUser = { ...prevUser, status: driverProfile.status };
                
                // Lưu đè lại vào bộ nhớ máy để lần sau vô app nó nhớ trạng thái này
                AsyncStorage.setItem('userInfo', JSON.stringify(updatedUser)).catch(err => console.log(err));
                
                return updatedUser;
            });

        } catch (error) {
            console.error('Lỗi khi checkAuth:', error);
        }
    };

...
export const updateOrderStatus = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const result = await MerchantService.updateOrderStatus(req.user.id, Number(orderId), status);

        // 2. --- LOGIC REAL-TIME THÔNG BÁO ---
        
        // TRƯỜNG HỢP A: Quán bấm "Tìm tài xế" -> Báo cho Tài xế
        if (status === 'FINDING_DRIVER') {
            req.io.to('drivers_room').emit('job_available', {
        ...
        req.io.to(customerRoom).emit('order_status_update', {
        ...
        return AppResponse.success(res, result, 'Cập nhật trạng thái thành công!');
    } catch (error) {
        next(error);
    }
